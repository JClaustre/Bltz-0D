...
initIl=`awk "BEGIN{ print 1.0 }" `; maxIl=`awk "BEGIN{ print 2.0}" ` ; NIlsimu=`awk "BEGIN{ print 3 }" `
initAr=`awk "BEGIN{ print 4E-5}" `; maxAr=`awk "BEGIN{ print 2E-5}" `; NArsimu=`awk "BEGIN{ print 6 }" `
...
do
	...
	do
	    ...
	    int=$(printf '%.1f' "$Pr")
	    int2=$(printf '%.1f' "$Il")
	    sed -i "$ln s/$ligne/$int\_Torr\_1.1487_W/" SRC_F90/param.f90
            echo "Pressure test : " $ligne, $int, $Pr

	    echo "Valeur de Pression  : $Pr et Critere d'arret : $Ar"
	    echo "########## Valeur de Polarisation  : $Po ##########"
	    # **** Update Polarization
	    ln=$(sed -n "/Tagada/=" SRC_F90/pumping.f90)
	    ligne=`awk 'NR==131 { print $3 }' SRC_F90/pumping.f90`
	    sed -i "$ln s/$ligne/$Po/" SRC_F90/pumping.f90

	    # **** Copie des fichier Steady State vers le repertoire pour le calcul des Ai:
	    mkdir ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/
	    cp -r ./datFile/MEOP/$int\_Torr\_1.1487_W/* ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/.
	    # **** Changement des variable dans le fichier Rstart ************************************************
	    # **** Laser On
	    Torr=`awk 'NR==18 { print $1}' ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he`
	    ligne=`awk "BEGIN{ print 18 }" `
	    sed -i "$ligne s/$Torr/1/" datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he
	    # **** Laser Power
	    Torr=`awk 'NR==21 { print $1}' ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he`
	    ligne=`awk "BEGIN{ print 21 }" `
	    sed -i "$ligne s/$Torr/$int2\e+00/" datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he
	    # **** Laser Section (m2)
	    Torr=`awk 'NR==22 { print $1}' ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he`
	    ligne=`awk "BEGIN{ print 22 }" `
	    sed -i "$ligne s/$Torr/0.001963/" datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he
	    # **** Laser Time Start
	    Torr=`awk 'NR==26 { print $1}' ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he`
	    int3=$(printf '%.8f' "$Torr")
	    Torr2=`awk 'NR==23 { print $1}' ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he`
	    ligne=`awk "BEGIN{ print 23 }" `
	    sed -i "$ligne s/$Torr2/$int3/" datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he
	    # **** end Time of simulation 
	    Torr=`awk 'NR==13 { print $1}' ./datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he`
	    Torr2=$(awk "BEGIN{print $int3+$Ar}")
	    ligne=`awk "BEGIN{ print 13 }" `
	    sed -i "$ligne s/$Torr/$Torr2/" datFile/MEOP/Calcul_tr_tdep/$int\_Torr\_1.1487_W/Rstart/Rs_input_he
	    # **** Change time in "evolution.f90" 
	    ln=$(sed -n "/Tagada/=" SRC_F90/evolution.f90)
	    Torr=`awk -F "GT." 'NR==75 { print $2}' SRC_F90/evolution.f90`
	    echo "il y a ecrit: $Torr"

	    Torr2=$(awk "BEGIN{print $int3+$Ar/10.0}")
