...
initPr=`awk "BEGIN{ print 0.3 }" ` ; maxPr=`awk "BEGIN{ print 2.2 }" `; NPrsimu=`awk "BEGIN{ print 4 }" `
...
do
    ...
    int=$(printf '%.1f' "$Pr")
    int2=$(printf '%.1f' "$Il")
    sed -i "$ln s/$ligne/$int\_Torr\_5mW\_$int2\_W/" SRC_F90/param.f90
	...
	mkdir ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/
	cp -r ./datFile/MEOP/$int\_Torr\_5mW/* ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/.
	# **** Changement des variable dans le fichier Rstart ************************************************
	# **** Laser On
	Torr=`awk 'NR==18 { print $1}' ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he`
	ligne=`awk "BEGIN{ print 18 }" `
	sed -i "$ligne s/$Torr/1/" datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he
	# **** Laser Power
	Torr=`awk 'NR==21 { print $1}' ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he`
	ligne=`awk "BEGIN{ print 21 }" `
	sed -i "$ligne s/$Torr/$int2\e+00/" datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he
	# **** Laser Section (m2)
	Torr=`awk 'NR==22 { print $1}' ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he`
	ligne=`awk "BEGIN{ print 22 }" `
	sed -i "$ligne s/$Torr/0.001963/" datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he
	# **** Laser Time Start
	Torr=`awk 'NR==26 { print $1}' ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he`
	int3=$(printf '%.4f' "$Torr")
	Torr2=`awk 'NR==23 { print $1}' ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he`
	ligne=`awk "BEGIN{ print 23 }" `
	sed -i "$ligne s/$Torr2/$int3/" datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he
	# **** end Time of simulation 
	Torr=`awk 'NR==13 { print $1}' ./datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he`
	Torr2=$(awk "BEGIN{print $int3+$Ar}")
	ligne=`awk "BEGIN{ print 13 }" `
	sed -i "$ligne s/$Torr/$Torr2/" datFile/MEOP/Calcul_Ai_P/$int\_Torr\_5mW\_$int2\_W/Rstart/Rs_input_he
	# ****************************************************************************************************
	make clean
	make
	./run_BOD
