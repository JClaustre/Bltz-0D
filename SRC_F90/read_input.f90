...
!    END do
  ...
  SUBROUTINE Init (sys, clock, ion, elec, meta, lasr)
    ...
    IF (Clock%Rstart == 0) THEN                                             !
       ...
       END SELECT                                                           !
       DO i = 1, NumMeta                                                    !
          IF (i.EQ.1) meta(i)%Ni = 0.0d+15     ! Metastable 2S3             !
          IF (i.EQ.2) meta(i)%Ni = 0.0d+13     ! Metastable 2S1             !
          IF (i.EQ.3) meta(i)%Ni = 0.0d+14     ! Radiative state 2P3        !
          IF (i.GE.4) meta(i)%Ni = 0.0d+06                                  !
       END DO                                                               !
